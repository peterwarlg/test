...
void robot_init()
{
  ...
  // 从{B}坐标系到{H}坐标系的转换
  easyMat_create(&robot.B_U_H, 3, 3);
  easyMat_eye(&robot.B_U_H);
  // 从{H}坐标系到{B}坐标系的转换
  easyMat_create(&robot.H_U_B, 3, 3);
  easyMat_eye(&robot.H_U_B);
  // 关节空间
...
void robot_free()
{
  easyMat_free(&robot.B_U_H);
  easyMat_free(&robot.H_U_B);
...
void updateRobotState()
{
  ...
  easyMat_RPY(&robot.B_U_H, robot.eulerAngle.roll, robot.eulerAngle.pitch, robot.eulerAngle.yaw);
  easyMat_trans(&robot.H_U_B, &robot.B_U_H);
  /*弹簧长度及其导数更新*/
...
void robot_control()
{
  ...
  if (robot.stateMachine == FLIGHT)
  {
    ...
    // 转到{B}坐标系下
    easyMat_mult(&robot.workPoint_B_desire, &robot.H_U_B, &robot.workPoint_H_desire);
    // 计算期望关节角
...
void update_xz_dot()
{
  ...
  easyMat_mult(&robot.workPoint_H, &robot.B_U_H, &robot.workPoint_B);
